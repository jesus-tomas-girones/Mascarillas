...
import static es.upv.master.android.reconocimientofacial.data.DataBase.COLLECTION;
...
public class LabelActivity extends AppCompatActivity implements View.OnTouchListener {
   ...
   //TODO tratar de pasar parte de esta funcion a Firebase con parámetros adecuados
   public void saveLabels() {
      ...
      for (int i = 0; i < circle.length; i++) {
         if (circle[i].getVisibility() == View.VISIBLE) {
            DataBase.updateLabel(listLabel.get(i), getX(circle[i]), getY(circle[i]));
/*
...
//      PhotoRef.update("labelled", true);
      DataBase.updateLabel(String id, String[] label, float[] x, float[] y) ;
   }
