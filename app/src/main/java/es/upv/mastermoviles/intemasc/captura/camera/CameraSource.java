package es.upv.mastermoviles.intemasc.captura.camera;
...
public class CameraSource {
    ...
    private static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight) {
        ...
        ArrayList<Integer> listIndexPair = new ArrayList<Integer>();
        int minDiff = Integer.MAX_VALUE;
        int pairABuscar = desiredWidth*desiredHeight;
        for (SizePair sizePair : validPreviewSizes) {
            ...
            int index = size.getWidth()*size.getHeight()/pairABuscar;
            listIndexPair.add(index);
        }
        selectedPair = validPreviewSizes.get(masCercano(listIndexPair, pairABuscar));
        return selectedPair;
    }

    public static int masCercano(ArrayList<Integer> numeros, int num)
    {
        int cercano = 0;
        for (int i = 0; i < numeros.size(); i++)
        {
            ...
            if (numeros.get(i) > num /*|| numeros.get(i) < num*/)
            {
                //cercano = numeros.get(i);
                if (cercano > numeros.get(i))
                {
                    cercano = numeros.get(i);
                }
                else{
        ...
        return numeros.indexOf(cercano);
    }
